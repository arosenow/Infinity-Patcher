...
def on_button_click():
    ...
    for file in os.listdir(file_path): 
         if file == modname+".tp2": modexists = 1
         if file == modname+".ini": inifound = 1

    if modexists == 1:
        with open(file_path+'/'+modname +'.tp2') as f:
            lines = f.readlines()
            ...
            for i3 in range(len(modules)):
                if label_found[i3] == 0:
                    lines[modules[i3]] = lines[modules[i3]] + '\n\tLABEL ~' + modname + 'mc' + str(i3+ 1) + '~'
                    print(lines[modules[i3]])

            save_lines.extend(lines)
        
        with open(file_path+'/'+modname +'.tp2','w') as f:
        ...
        if inifound == 1:
            ...
            with open(file_path+'/'+modname +'.ini') as f:
                ...
                if foundlabels == 0: 
                    ...
                    if metadata > 0: editedini[metadata-1] = editedini[metadata-1] + '\nLabelType = GloballyUnique'
